...
module.exports = {
  data: {
    name: "designerRefuseReasonModal",
  },
  async execute(interaction, client) {
    const embedDesigner = EmbedBuilder.from(interaction.message.embeds[0]);
    ...
    embedDesigner
      .setColor("#8c0000")
      .setTitle(
        "*Inscrição Recusada* - **Designer**"
      )
      .setImage(guildFormBanner)
      .addFields([
        {
          name: "**__Recusado pelo motivo:__**",
          value: `${reasonInput || "O motivo não foi definido."}`,
          inline: true,
        },
        {
          name: "**__Recusado por:__**",
          value: `${interaction.user} (${interaction.user.id})`,
          inline: true,
        },
      ]);

    await interaction.message.edit({ embeds: [embedDesigner], components: [] });
      ...
      await member
        ...
        .catch(async () => {
          await interaction.reply({
            content: `<a:1_laydowntorest:1042548347715727390>` + " **|** **Erro 404:** O contacto com o membro não pôde ser estabelecido.",
            ephemeral: true,
          });
        });
    }
