...
module.exports = {
  ...
  async execute(interaction, client) {
    ...
    if (member) {
      await member.roles.add(config.devID);
    ...
    const content = new EmbedBuilder()
      ...
      .setDescription(
        `Olá ${member},\n\n` +
          "Agradecemos pelo seu interesse na Black Lotus. Recebemos um grande número de inscrições e, após uma ***revisão cuidadosa***, **temos o prazer de informar que você foi selecionado** para avançar para a próxima etapa como **Desenvolvedor** em nossa associação.\n\n" +
      ...
      await member
        ...
        .catch(async () => {
          await interaction.reply({
            content:
            `<a:1_laydowntorest:1042548347715727390>` + " **|** **Erro 404:** O contacto com o membro não pôde ser estabelecido.",
            ephemeral: true,
          });
        });
    }
    ...
    embed
      .setColor("#00910f")
      .setTitle(`*Inscrição aprovada* - **Desenvolvedor**`)
      .addFields({
        name: "**__Aprovado por:__**",
    ...
    await interaction.editReply({
      content: `<a:1_everythingisstable:1042548343852777523>` + " **|** Inscrição aprovada: **Desenvolvedor** **|** *O usuário será notificado e receberá o cargo de* ***Desenvolvedor***.",
      ephemeral: true,
    });
    ...
    await interaction.client
      ...
      .then(async (webhook) => {
        await webhook.send({
          content: `<a:1_verified:1055293708825808947> **|** O membro ${member} teve sua inscrição *analisada e aprovada*, e agora faz oficialmente parte da **Lotus UNIT** na categoria ***Desenvolvedor***. Seja muito bem-vindo!`,
          username: "Lotus UNIT",
