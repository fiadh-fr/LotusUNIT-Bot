...
module.exports = {
  ...
  async execute(interaction, client) {
    const member = await interaction.guild.members
      ...
      if (member) {
        await Promise.all([
          member.roles.add(config.designerID),
          member.roles.remove(config.awaitID)
        ]);
      } else
        return interaction[
          interaction.replied || interaction.deferred ? "followUp" : "reply"
        ]({
          content: `<a:1_laydowntorest:1042548347715727390>` + " **|** **Erro 500:** Membro não encontrado. Não foi possível atribuir-lhe o cargo.",
          ephemeral: true,
        });
    ...
    const content = new EmbedBuilder()
        ...
        .setDescription(
          `Olá ${member},\n\n` +
            "Agradecemos pelo seu interesse na Black Lotus. Recebemos um grande número de inscrições e, após uma ***revisão cuidadosa***, **temos o prazer de informar que você foi selecionado** para avançar para a próxima etapa como **Designer** em nossa associação.\n" +
      ...
      embed
      .setColor("#00910f")
      .setTitle(`*Inscrição aprovada* - **Designer**`)
      .addFields({
        name: "Aprovado por",
      ...
      await interaction.editReply({
        content:`<a:1_everythingisstable:1042548343852777523>` + " **|** Inscrição aprovada: **Designer** **|** *O usuário será notificado e receberá o cargo de* ***Designer***.",
        ephemeral: true,
      });
      ...
      await interaction.client
        ...
        .then(async (webhook) => {
          await webhook.send({
            content: `<a:1_verified:1055293708825808947> **|** O membro ${member} teve sua inscrição *analisada e aprovada*, e agora faz oficialmente parte da **Lotus UNIT** na categoria ***Designer***. Seja muito bem-vindo!`,
