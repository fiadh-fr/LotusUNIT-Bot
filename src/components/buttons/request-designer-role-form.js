...
module.exports = {
  data: {
    name: "request-designer-role-form",
  },

  async execute(interaction, client) {
    const designerConfirmationEmbed = new EmbedBuilder()
    ...
    const reply = await interaction.reply({
      embeds: [designerConfirmationEmbed],
      components: [row],
      ephemeral: true,
      fetchReply: true,
    });
    ...
    collector.on("collect", async (i) => {
      if (i.customId === "collector:confirm") {
        const modal = new ModalBuilder()
          .setCustomId(`designerRequestModal`)
          .setTitle(`Lotus UNIT - Designer`);

        const portfolioLink = new TextInputBuilder()
          .setCustomId(`portfolioLink`)
          .setLabel(`Forneça o link do seu portfólio`)
          .setRequired(true)
          .setMinLength(10)
        ...
        const experienceInfo = new TextInputBuilder()
          ...
          .setLabel(`Qual é a sua experiência na área?`)
          .setRequired(true)
          .setPlaceholder("Website, banners, logotipos, emojis, etc...")
          .setMinLength(5)
          .setStyle(TextInputStyle.Paragraph);

        const styleInfo = new TextInputBuilder()
          .setCustomId(`styleInfo`)
          .setLabel(`Qual é o seu estilo na área?`)
          .setRequired(true)
          .setPlaceholder(
            "Estilo anime/manga, geek, medieval, estilos variados, etc..."
          )
          .setMinLength(5)
          .setStyle(TextInputStyle.Paragraph);
        ...
        const experienceTime = new TextInputBuilder()
          ...
          .setLabel(`Há quanto tempo trabalha na área?`)
          .setRequired(true)
          .setMinLength(5)
          .setStyle(TextInputStyle.Short);
        ...
        const serverInvite = new TextInputBuilder()
          ...
          .setLabel(`Convite para o seu servidor.`)
          .setPlaceholder("(Caso não haja, deixe o campo em branco.")
          .setStyle(TextInputStyle.Short)
          .setRequired(false);

        modal.addComponents(
          new ActionRowBuilder().addComponents(portfolioLink)
        );
        modal.addComponents(
          new ActionRowBuilder().addComponents(experienceInfo)
        );
        modal.addComponents(new ActionRowBuilder().addComponents(styleInfo));
        modal.addComponents(
          new ActionRowBuilder().addComponents(experienceTime)
        );
        modal.addComponents(new ActionRowBuilder().addComponents(serverInvite));
